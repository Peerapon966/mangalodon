...
docker buildx build --platform linux/amd64 --provenance=false --no-cache -t ${aws_ecr_repository.repo[each.key].repository_url}:${local.sha1_hashes[each.key]} --build-arg ENV=${var.environment} --push ../src/${each.key}
%{else}
